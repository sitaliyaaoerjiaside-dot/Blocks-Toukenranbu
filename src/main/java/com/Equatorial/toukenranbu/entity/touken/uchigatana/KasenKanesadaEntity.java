...
public class KasenKanesadaEntity extends ToukenDanshiEntity {

    public KasenKanesadaEntity(EntityType<? extends TamableAnimal> entityType, Level level) {
    ...
    protected String getEntityNameKey() {
        return "kasen_kanesada";
    }

    public static AttributeSupplier setAttributes() {
        return TamableAnimal.createMobAttributes()
                .add(Attributes.MAX_HEALTH, 56.0D)// 初始生命值（会被 baseMaxHealth + 刀装加成覆盖）
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        controllers.add(new AnimationController<>(this, "controller", 0, event -> {
            if (this.isInSittingPose()) {
                event.getController().setAnimation(RawAnimation.begin().thenLoop("animation.kasen_kanesada.sit"));
                return PlayState.CONTINUE;
            }
            if (event.isMoving()) {
                event.getController().setAnimation(RawAnimation.begin().then("animation.kasen_kanesada.walk", Animation.LoopType.LOOP));
                return PlayState.CONTINUE;
            }
            event.getController().setAnimation(RawAnimation.begin().thenLoop("animation.kasen_kanesada.idle"));
            return PlayState.CONTINUE;
        }));
        controllers.add(new AnimationController<>(this, "attackController", 0, event -> {
            if (this.swinging) {
                event.getController().setAnimation(RawAnimation.begin().then("animation.kasen_kanesada.attack", Animation.LoopType.PLAY_ONCE));
                return PlayState.CONTINUE;
            }
    ...
    protected String getDeathMessageKey() {
        return "death.toukenranbu_mod.kasen_kanesada";
    }
    // ============================================================
