...
public class NaginataEntity extends Monster implements GeoEntity, JikkoEntity {
    private AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);

    public NaginataEntity(EntityType<? extends Monster> pEntityType, Level pLevel) {
    ...
    private <E extends NaginataEntity> PlayState predicate(final AnimationState<E> event) {
        if (event.isMoving()) {
            event.getController().setAnimation(RawAnimation.begin().then("animation.naginata.walk", Animation.LoopType.LOOP));
            return PlayState.CONTINUE;
        }
        event.getController().setAnimation(RawAnimation.begin().thenLoop("animation.naginata.idle"));
        return PlayState.CONTINUE;
    }

    private <E extends NaginataEntity> PlayState attackPredicate(final AnimationState<E> event) {
        if (this.swinging) {
            event.getController().setAnimation(RawAnimation.begin().then("animation.naginata.attack", Animation.LoopType.PLAY_ONCE));
            this.swinging = false;
        }
        return PlayState.CONTINUE;
    }
    ...
    public void registerControllers(final AnimatableManager.ControllerRegistrar controllers) {
        ...
        controllers.add(new AnimationController<>(this, "attackController", event -> PlayState.STOP)
                .triggerableAnim("attack", RawAnimation.begin()
                        .then("animation.naginata.attack", Animation.LoopType.PLAY_ONCE)));
    }
    ...
    protected void playStepSound(BlockPos pos, BlockState blockIn) {
        this.playSound(SoundEvents.DROWNED_STEP, 1.0F, 1.0F);
    }
    ...
    @Override
    protected SoundEvent getHurtSound(DamageSource damageSourceIn) { return SoundEvents.DROWNED_HURT; }
    ...
    private void applyNightModifier() {
        // 根据刀种改这两个系数
        this.getAttribute(Attributes.ATTACK_DAMAGE).setBaseValue(this.baseDayAttack * 0.5); // 攻击
        this.getAttribute(Attributes.MOVEMENT_SPEED).setBaseValue(this.baseDaySpeed * 0.7);  // 速度
    }
