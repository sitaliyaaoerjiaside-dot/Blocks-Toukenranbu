...
public class WakizashiEntity extends Monster implements GeoEntity, JikkoEntity {
    private AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);

    public WakizashiEntity(EntityType<? extends Monster> pEntityType, Level pLevel) {
    ...
    private <E extends WakizashiEntity> PlayState predicate(final AnimationState<E> event) {
        if (event.isMoving()) {
            event.getController().setAnimation(RawAnimation.begin().then("animation.wakizashi.walk", Animation.LoopType.LOOP));
            return PlayState.CONTINUE;
        }
        event.getController().setAnimation(RawAnimation.begin().thenLoop("animation.wakizashi.idle"));
        return PlayState.CONTINUE;
    }

    private <E extends WakizashiEntity> PlayState attackPredicate(final AnimationState<E> event) {
        if (this.swinging) {
            event.getController().setAnimation(RawAnimation.begin().then("animation.wakizashi.attack", Animation.LoopType.PLAY_ONCE));
            this.swinging = false;
        }
        return PlayState.CONTINUE;
    }
    ...
    private void applyNightModifier() {
        // 根据刀种改这两个系数
        this.getAttribute(Attributes.ATTACK_DAMAGE).setBaseValue(this.baseDayAttack * 1.5); // 攻击
        this.getAttribute(Attributes.MOVEMENT_SPEED).setBaseValue(this.baseDaySpeed * 1.2);  // 速度
    }
