...
public class TachiEntity extends Monster implements GeoEntity, JikkoEntity {
    private AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);

    public TachiEntity(EntityType<? extends Monster> pEntityType, Level pLevel) {
    ...
    private <E extends TachiEntity> PlayState predicate(final AnimationState<E> event) {
        if (event.isMoving()) {
    ...
    private <E extends TachiEntity> PlayState attackPredicate(final AnimationState<E> event) {
        if (this.swinging) {
    ...
    private void applyNightModifier() {
        // 根据刀种改这两个系数
        this.getAttribute(Attributes.ATTACK_DAMAGE).setBaseValue(this.baseDayAttack * 0.6); // 攻击
