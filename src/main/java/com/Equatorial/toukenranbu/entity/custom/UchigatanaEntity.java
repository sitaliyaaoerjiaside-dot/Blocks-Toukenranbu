...
public class UchigatanaEntity extends Monster implements GeoEntity, JikkoEntity {
    private AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);

    public UchigatanaEntity(EntityType<? extends Monster> pEntityType, Level pLevel) {
    ...
    private <E extends UchigatanaEntity> PlayState predicate(final AnimationState<E> event) {
        if (event.isMoving()) {
            event.getController().setAnimation(RawAnimation.begin().then("animation.uchigatana.walk", Animation.LoopType.LOOP));
            return PlayState.CONTINUE;
        }
        event.getController().setAnimation(RawAnimation.begin().thenLoop("animation.uchigatana.idle"));
        return PlayState.CONTINUE;
    }

    private <E extends UchigatanaEntity> PlayState attackPredicate(final AnimationState<E> event) {
        if (this.swinging) {
            event.getController().setAnimation(RawAnimation.begin().then("animation.uchigatana.attack", Animation.LoopType.PLAY_ONCE));
            this.swinging = false;
        }
        return PlayState.CONTINUE;
    }
    ...
    protected void playStepSound(BlockPos pos, BlockState blockIn) {
        this.playSound(SoundEvents.HUSK_STEP, 1.0F, 1.0F);
    }
    ...
    @Override
    protected SoundEvent getHurtSound(DamageSource damageSourceIn) { return SoundEvents.HUSK_HURT; }
    ...
    private void applyNightModifier() {
        // 根据刀种改这两个系数
        this.getAttribute(Attributes.ATTACK_DAMAGE).setBaseValue(this.baseDayAttack * 1.2); // 攻击
        this.getAttribute(Attributes.MOVEMENT_SPEED).setBaseValue(this.baseDaySpeed * 0.95);  // 速度
    }
